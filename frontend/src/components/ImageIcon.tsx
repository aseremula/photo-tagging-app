function ImageIcon({ imagePath="", markAsFound=false } : { imagePath: string, markAsFound: boolean }) {
  ...
  return (
    <div className={`min-w-13 min-h-13 max-w-13 max-h-13 pointer-events-none rounded-sm overflow-hidden flex items-center justify-center ${imagePath==="" && "bg-(--light-red)"}`}>
    
    {/* If image path exists, show image. Otherwise, show ? */}
    {(imagePath !== "") ?
      <>
        <img className={`relative ${(markAsFound) && "grayscale opacity-20"}`} src={imagePath} width="52px" height="52px" alt="A snippet from eBoy's San Francisco Pixorama"/>

        {(markAsFound) && <svg className="absolute w-10 h-10 fill-(--light-red) pointer-events-none" xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960"><path d="M382-208 122-468l90-90 170 170 366-366 90 90-456 456Z"/></svg>}
      </>
      :
